	...
	.controller('desafioController', function (clientService) {
		...
		var dCtrl = this,
			...
			init = function () {
				dCtrl.owner    = 'jsanta';
				dCtrl.editData = {};
				dCtrl.newData  = { owner: dCtrl.owner };

				// Carga el listado de clientes desde el arreglo de datos simulados
				dCtrl.clientList = clientService.retrieveClients();

				// Laa variable visibleClientsList hace una copia de 10 elementos segun la pagina
				// que se esté visitando
				// IMPORTANTE: Para poder hacer esta copia primero se debe asegurar que efectivamente
				// se han rescatado los datos desde la API Rest, por eso se utiliza la promesa.
				dCtrl.clientList.$promise.then(function () {
		...
		dCtrl.updateClientInfo = function (updatedClient) {
			...
			clientService.updateClient(updatedClient, function (res) {
		...
		dCtrl.createClient = function (newClient) {
			console.log('createClient: ', newClient);
			clientService.createClient(newClient, function (res) {
		...
		dCtrl.cleanForm = function (event) {
			...
			console.log(event);
			if (event.target.form.id === 'createFrm') {
				// Copia valores nulos en los campos editables
				angular.extend(dCtrl.newData, {
					owner: dCtrl.owner,
					name: null,
					lastname: null,
					email: null,
		...
		dCtrl.deleteClient = function (client, idx) {
			...
			if (confirm('¿Esta seguro de querer eliminar el cliente ' + dCtrl.clientList[idx].name + ' ' + dCtrl.clientList[idx].lastname + '?')) {
				...
				clientService.deleteClient(dCtrl.clientList[idx], function (res) {
